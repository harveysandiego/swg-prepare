...
printf --  "\nInstalling Oracle Instantclients\n"
...
printf --  "/usr/lib/oracle/18.3/client/lib" | tee -a /etc/ld.so.conf.d/oracle.conf
printf --  "export ORACLE_HOME=/usr/lib/oracle/18.3/client" | tee -a /etc/profile.d/oracle.sh
printf --  "export PATH=$PATH:/usr/lib/oracle/18.3/client/bin" | tee -a /etc/profile.d/oracle.sh
printf --  "export LD_LIBRARY_PATH=/usr/lib/oracle/18.3/client/lib:/usr/include/oracle/18.3/client" | tee -a /etc/profile.d/oracle.sh
ln -sf /usr/include/oracle/18.3/client $ORACLE_HOME/include
ldconfig
touch /etc/profile.d/java.sh
printf --  "export JAVA_HOME=/usr/lib/jvm/zulu-11-x86" | tee -a /etc/profile.d/java.sh